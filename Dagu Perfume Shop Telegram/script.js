...
(function () {
  ...
  const draw = () => {
    ...
    // Draw the massive stylish centerpiece bottle, now centered in hero-reveal
    const cx = W < 900 ? W * 0.5 : W * 0.42; // Center on mobile, slightly left on desktop
    const cy = (W < 900 ? H * 0.38 : H * 0.55) + fl + (mouseY / H - 0.5) * 30;
    const drawSize = Math.min(W, H) * (W < 500 ? 0.38 : 0.45);
    drawBottle(cx + (mouseX / W - 0.5) * 40, cy, drawSize, 0.95);
